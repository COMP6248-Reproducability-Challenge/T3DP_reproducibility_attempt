...
from .lbs import (
    lbs, vertices2landmarks, find_dynamic_lmk_idx_and_bcoords, batch_rodrigues, vertices2joints)
...
class SMPL(nn.Module):
    ...
    def __init__(self, model_path, data_struct=None,
                 ...
                 vertex_ids=None, joint_regressor_extra=None,
                 **kwargs):
        ...
        if 'effective_batch_size' in kwargs:
            self.effective_batch_size = kwargs['effective_batch_size']
        else:
        ...
        if joint_regressor_extra is not None:
            self.register_buffer('joint_regressor_extra', torch.tensor(pickle.load(open(joint_regressor_extra, 'rb'), encoding='latin1'), dtype=dtype))
    ...
    def forward(self, betas=None, body_pose=None, global_orient=None,
        ...
        batch_size = max(self.effective_batch_size, betas.shape[0], global_orient.shape[0],
                         body_pose.shape[0])
        ...
        if body_pose.shape[0] != batch_size:
            ...
            body_pose = body_pose.repeat(num_repeats, 1)
...
class SMPLH(SMPL):
    ...
    def forward(self, betas=None, global_orient=None, body_pose=None,
        ...
        if left_hand_pose.shape[0] != batch_size:
            num_repeats = int(batch_size / left_hand_pose.shape[0])
            left_hand_pose = left_hand_pose.repeat(num_repeats, 1)
        if right_hand_pose.shape[0] != batch_size:
            num_repeats = int(batch_size / right_hand_pose.shape[0])
            right_hand_pose = right_hand_pose.repeat(num_repeats, 1)

        if not pose2rot:
            left_hand_pose = batch_rodrigues(left_hand_pose.view(-1,3), dtype=left_hand_pose.dtype).view([batch_size, -1, 3, 3])
            right_hand_pose = batch_rodrigues(right_hand_pose.view(-1,3), dtype=right_hand_pose.dtype).view([batch_size, -1, 3, 3])
...
class SMPLX(SMPLH):
    ...
    def __init__(self, model_path,
                 ...
                 dtype=torch.float32,
                 ext='npz', joint_regressor_extra=None,
                 **kwargs):
    ...
    def forward(self, betas=None, global_orient=None, body_pose=None,
        ...
        if leye_pose.shape[0] != batch_size:
            num_repeats = int(batch_size / leye_pose.shape[0])
            leye_pose = leye_pose.repeat(num_repeats, 1)
        if reye_pose.shape[0] != batch_size:
            num_repeats = int(batch_size / reye_pose.shape[0])
            reye_pose = reye_pose.repeat(num_repeats, 1)
        if jaw_pose.shape[0] != batch_size:
            num_repeats = int(batch_size / jaw_pose.shape[0])
            jaw_pose = jaw_pose.repeat(num_repeats, 1)
        if expression.shape[0] != batch_size:
            num_repeats = int(batch_size / expression.shape[0])
            expression = expression.repeat(num_repeats, 1)
        ...
        lmk_faces_idx = self.lmk_faces_idx.unsqueeze(
            dim=0).repeat(batch_size, 1).contiguous()
        ...
        if self.use_face_contour:
            dyn_lmk_faces_idx, dyn_lmk_bary_coords = find_dynamic_lmk_idx_and_bcoords(
                ...
                self.neck_kin_chain, dtype=vertices.dtype)

            lmk_faces_idx = torch.cat([lmk_faces_idx,
                                       dyn_lmk_faces_idx], 1)
            lmk_bary_coords = torch.cat(
                [lmk_bary_coords.repeat(batch_size, 1, 1),
